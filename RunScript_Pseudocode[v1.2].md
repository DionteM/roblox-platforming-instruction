### RunScript - Pesudocode 

1. Create a `local` variable named `UserInputService` and set it to the following. `game` is an global instance it has a function inside of it called `:GetService()` use this function on the instance with `"UserInputService"` as an input

2. Connect to an event called `InputBegan`  that's inside of `UserInputService` with  `input` as an input. There's a **hint** below
```lua
